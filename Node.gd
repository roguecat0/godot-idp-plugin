...
func _ready() -> void:
	...
	var rip := kb.add_function("rip",Yolo,Yolo,{2:3,1:4})
	...
	var kb_str = kb.parse_to_idp()
	
	var kb2: KnowlegdeBase = IDP.create_empty_kb()
	var T := kb2.add_type("T",["1..3"],IDP.INT)
	var pred = kb2.add_predicate("pred",[T,T])
	var pred2 = kb2.add_predicate("pred2",T)
	var fun := kb2.add_function("fun",T,T)
	var fun2:= kb2.add_function("fun2",[T,T],T)
	kb2.theory.add_line("pred(1,1).")
	kb2.theory.add_line("pred2(1).")
	var con2 = kb2.add_constant("con2",T)
	#kb2.theory.add_line("~pred(1,1).")
